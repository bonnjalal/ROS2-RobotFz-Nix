...
class SingleBallDetector(Node):
    # Define the Orange/Red color range in HSV
    _orange_lower_hsv = np.array(
        # [0, 100, 100] # orange
        [48, 69, 0]  # blue
    )  # Example lower bound (adjust as needed)
    _orange_upper_hsv = np.array(
        # [10, 255, 255] # orange
        [138, 183, 168]  # blue
    ...
    def _filter_image_using_ball_color(self, image):
        ...
        mask = cv.inRange(hsv_image, self._orange_lower_hsv, self._orange_upper_hsv)

        # Apply morphological operations to reduce noise
        mask = cv.erode(mask, None, iterations=2)
        mask = cv.dilate(mask, None, iterations=2)

        self.get_logger().info("Shape of color mask: {}".format(mask.shape))
    ...
    def _detect_circles(image_1_channel):
        ...
        circles = cv.HoughCircles(
            ...
            dp=1.0,  # 1
            minDist=20,  # change this to see if detection improves
            param1=100,  # 100
            param2=30,  # 30
            minRadius=10,  # 10
            maxRadius=100,  # 0
    ...
    def detect_ball(self, image):
        ...
        self.get_logger().info("Starting detect_ball")
        ...
        self.get_logger().info(
            "Shape of filtered_color_binary_image: {}".format(
                filtered_color_binary_image.shape
            )
        )

        detected_circles = self._detect_circles(filtered_color_binary_image)
        if detected_circles is not None:
            self.get_logger().info("Raw Hough Circles: {}".format(detected_circles))
            detected_circles = convert_circles_properties_to_list_of_circles(
                detected_circles
            )
            self.get_logger().info("Converted Circles: {}".format(detected_circles))

            # Filter for circles based on area of detection
            filtered_circles = []
            ...
            if filtered_circles:
                self.get_logger().info(
                    "Found {num_of_circles} Circles".format(
                        num_of_circles=str(len(filtered_circles))
                    )
                )
                mean_circle_detected = calculate_mean_of_circles(filtered_circles)
                if mean_circle_detected:
                    mean_circle_detected_ints = (
                        mean_circle_detected.round_and_convert_values_to_int()
                    )
                    self._put_circles_on_image(
                        image=image,
                        weak_circles=filtered_circles,
                        strong_circles=(mean_circle_detected_ints,),
                    )
                    self.get_logger().info(
                        "Detected Mean Circle: {}".format(mean_circle_detected_ints)
                    )
...
def main(args=None):
    ...
    ball_detector = SingleBallDetector(
        ball_color_bgr=(
            ...
            32,
            142,
        ),  # This is irrelevant now, as you will be using HSV
